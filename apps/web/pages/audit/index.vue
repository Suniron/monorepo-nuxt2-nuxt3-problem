...
export default {
  components: { LoadingSpinner, SpiderGraph },
  data() {
    return {
      headers: [
        ...
        <v-stepper-content :key="`step-content-${i}`" :step="i + 1">
          ...
          >
            <template #[`item.title`]="{ item }">
              <p>
                <v-chip small>
                  {{ item.section }}
                </v-chip> <b>{{ item.title }}</b>:<br>{{ item.description }}
              </p>
            </template>
          ...
          <v-row justify="end" style="margin: 5px;">
            <v-col cols="12" xl="1" lg="2">
              <v-btn text @click="step = i">
                PREVIOUS
              </v-btn>
            </v-col>
            <v-col cols="12" xl="1" lg="2">
              <v-btn color="primary" @click="step = i + 2">
                NEXT
              </v-btn>
            </v-col>
          </v-row>
